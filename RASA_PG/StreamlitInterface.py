...
def should_use_rasa_nlu(user_message: str) -> bool:
    """Decide if we should fully rely on RASA NLU+CORE for this user query."""
    # Already in RASA flow - continue
    # In should_use_rasa_nlu
    if st.session_state.get("in_rasa_flow", False):
        return True
        
    # Keywords that trigger RASA
    upgrade_keywords = [
        "upgrade", "business class", "premium economy", 
        "frequent flyer miles", "seat selection", 
        "cash plus miles", "cash upgrade"
    ]
    
    # Negative phrases to exclude
    negative_phrases = [
        "don't upgrade", "no upgrade", "don't proceed", 
        "not interested", "cancel upgrade"
    ]
    
    user_message_lower = user_message.lower()
    
    # Check if previous intent was RASA-known (for follow-ups)
    last_intent = st.session_state.get("current_message", {}).get("intent")
    if last_intent in RASA_KNOWN_INTENTS:
        st.sidebar.write(f"Session Intent check: {last_intent}")
        return True

    # Check for matches
    has_upgrade_keyword = any(kw in user_message_lower for kw in upgrade_keywords)
    has_negative = any(np in user_message_lower for np in negative_phrases)
    
    return has_upgrade_keyword and not has_negative


def invoke_rasa(message_text: str, sender_id: str = "default") -> tuple[str, str | None]:
    
    rasa_response = "Sorry, I didn't understand that."
...
def log_rasa_debug(message):
    ...
    if DEBUG_MODE:
        ...
        print(f"RASA DEBUG ABC: {message}")  # Also prints to console
...
if user_input and st.session_state.display_stage == 0:
    ...
    if assistant_response.startswith(("1.", "2.", "- ")) or "\n-" in assistant_response:
