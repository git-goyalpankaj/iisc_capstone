from dbUtils import connect_to_mysql, is_db_connected, select_record, delete_record 
...
def check_if_pnr_exists(pnr_number):
    ...
    print("[INFO DB QUERY] : ", query)
    result = select_record(query)
...
def get_pnr_details(pnr_number):
    ...
    user_data = select_record(query_to_fetch_user_details)
    flight_data = select_record(query_to_fetch_flight_details)
...
def cancel_ticket(pnr_number):
    ...
    if travel_date < today:
        ...
        delete_result = delete_record(delete_query)
        query_to_fetch_user_details = "select Name from user_data where id = '{user_id}'".format(user_id=user_id)
